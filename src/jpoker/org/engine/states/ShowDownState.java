...
import jpoker.org.poker.api.core.ManoEvaluador;
import jpoker.org.poker.api.game.Hand7Evaluator;
...
public class ShowDownState implements IState<ModelContext>{
    ...
    private List<PlayerEntity> calculateHandValue(List<Carta> cc, List<PlayerEntity> players){
        Hand7Evaluator evaluator=new Hand7Evaluator(new ManoEvaluador());
    ...
    public boolean execute(ModelContext model){
        ...
        //Collections.sort(inverseSortBets, (10,11) -> Long.compare(10, 11));
        ...
        Set<Long> bet4Analysis = players.stream()
                .filter(p->p.getState()==TexasHoldEmUtil.PlayerState.ALL_IN)
        ...
        while (!inverseSortBets.isEmpty()) {            
            ...
            if (bet4Analysis.contains(bet)) {
                ...
                currentPlayers.stream()
                        .filter(p -> p.getState() != TexasHoldEmUtil.PlayerState.OUT)
